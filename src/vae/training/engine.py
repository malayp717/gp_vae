...
from vae.evaluation import compute_image_metrics
from vae.losses import VAELoss
from vae.models import build_model
from vae.reporting import (
    DIVIDER,
    HEADER,
    append_train_stats,
    append_val_stats,
...
from vae.visualization import save_kl_per_dim_artifacts, save_validation_images

from .beta_schedule import BetaScheduler
from .checkpoints import load_training_state, save_checkpoint
...
def train(
    ...
    """Run full training and return the path to the latest checkpoint."""
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if save_interval and (epoch + 1) % save_interval == 0:
            ckpt_path = checkpoint_dir / f"{model_type}_epoch_{epoch + 1:04d}.pt"
            save_checkpoint(
                ckpt_path,
                model,
                optimizer,
    ...
    if latest_ckpt_path is None:
        latest_ckpt_path = checkpoint_dir / f"{model_type}_epoch_{epoch + 1:04d}.pt"
        save_checkpoint(
            latest_ckpt_path,
            model,
            optimizer,

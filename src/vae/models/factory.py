...
def build_model(config: dict[str, Any]) -> torch.nn.Module:
    """Construct a VAE or GP-VAE from config."""
    ...
    if model_type == "gp_vae":
        return GeneralizedPosteriorVAE(
            ...
            patch_div=model_cfg.get("patch_div", 4),
            in_channels=data_cfg.get("in_channels", 3),
            latent_dim=model_cfg.get("latent_dim_per_patch", 32),
    ...
    if model_type != "vae":
        raise ValueError(f"Unknown model type: {model_type!r}. Supported: 'vae', 'gp_vae'.")
    return VAE(
        in_channels=data_cfg.get("in_channels", 3),
        latent_dim=model_cfg.get("latent_dim", 512),
        encoder_channels=model_cfg.get("encoder_channels", [32, 64, 128, 256, 512]),
        decoder_channels=model_cfg.get("decoder_channels", [512, 256, 128, 64, 32]),
        image_size=data_cfg.get("image_size", 224),
